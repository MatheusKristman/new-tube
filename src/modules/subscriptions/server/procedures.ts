...
export const subscriptionsRouter = createTRPCRouter({
  getMany: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      // NOTE: Remove the last item if there is more data
      const items = hasMore ? data.slice(0, -1) : data;
      // NOTE: Set the next cursor to the last item if there is more data
  ...
  create: protectedProcedure.input(z.object({ userId: z.string() })).mutation(async ({ input, ctx }) => {
    const { userId } = input;
